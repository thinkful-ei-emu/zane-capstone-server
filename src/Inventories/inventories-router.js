...
inventoriesRouter.use(requireAuth);
// .route('/')

inventoriesRouter.post("/", jsonBodyParser, (req, res, next) => {
  const {
    item_name,
    description,
    quantity,
  ...
  InventoriesService.insertItem(req.app.get("db"), newItem)
    .then(item => {
      res
        .status(201)
        .location("/:itemId")
...
inventoriesRouter.get("/", (req, res, next) => {
  InventoriesService.getItemsFromTable(req.app.get("db"), req.user.id).then(
    items => res.status(200).json(items)
  );
  // .catch(next)
});
inventoriesRouter.delete("/:id", (req, res, next) => {
...
inventoriesRouter.put("/:id", (req, res) => {
  const {
    item_name,
    description,
    quantity,
...
inventoriesRouter.route("/:itemId").get((req, res, next) => {
  const { itemId } = req.params;
  return InventoriesService.getById(req.app.get("db"), itemId).then(item => {
    if (!item) {
